...
// Conturile probei se nasc pe traseul REAL („parola nu există": cont → link → omul își pune parola).
// Lipsea de la refacerea din 16.09, iar proba crăpa cu `puneParola is not defined` — dar abia după
// a patra verificare, deci raportul arăta „4/5 trecute" și părea aproape bună.
...
(async () => {
  ...
  try {
    ...
    await puneParola((await req(S, 'POST', '/api/users', { username: 'spion@b.ro', role: 'viewer', company_id: coB.id, full_name: 'Spion B' })).body, 'Parola123!', B);
    const U = jar();
    ...
    const A = jar();
    await puneParola((await req(S, 'POST', '/api/users', { username: 'admin@a.ro', role: 'company_admin', company_id: coA.id, full_name: 'Admin A' })).body, 'Parola123!', B);
