...
(async () => {
  ...
  check('GET /api/me → role superadmin', me.body.role === 'superadmin', me.body.role);
  const devAdmin = await api('GET', '/api/devices', A);
  check('super-admin vede ambele device-uri seedate (>=2)', Array.isArray(devAdmin.body) && devAdmin.body.length >= 2, devAdmin.body && devAdmin.body.length);

  console.log('\n— Companie + asignare device (multi-tenant) —');
  const company = (await api('POST', '/api/companies', { cookie: admin.cookie, body: { name: 'RBAC Test ' + Date.now() } })).body;
  check('companie creată', company && company.id, company);
  ...
  const client = await login('client.test.' + company.id + '@test.ro', 'Curcubeu7Vara');
